...
variable vsphere_vm_definition {
  description = "Defines the specs used to deploy the virtual machines"
  type = list(object({
    vm_name     = string  # The name that will be assigned to the virtual machine
    num_cpus    = number  # Number of vCPUs the VM will be given
    memory      = number  # Amount of memory (GB) the VM will be given
  }))
}